...
module.exports = createCoreService('api::moderation-log.moderation-log', ({ strapi }) => ({
  /**
   ...
   */
  async createLog({ action, contentType, contentId, moderatorId, reason, metadata }) {
    try {
      const log = await strapi.entityService.create('api::moderation-log.moderation-log', {
        data: {
          action,
          contentType,
          contentId,
      ...
      strapi.log.info(`Log de moderação criado: ${action} em ${contentType} #${contentId} por moderador #${moderatorId}`);
      return log;
    } catch (error) {
  ...
  async getLogsByModerator(moderatorId, options = {}) {
    ...
    return await strapi.entityService.findPage('api::moderation-log.moderation-log', {
      filters: {
        moderator: {
          id: moderatorId,
        },
  ...
  async getLogsByContent(contentType, contentId) {
    return await strapi.entityService.findMany('api::moderation-log.moderation-log', {
      filters: {
        contentType,
        contentId,
      },
