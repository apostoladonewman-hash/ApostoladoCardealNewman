  ...
  ({ strapi }) => ({
    ...
    async findByModerator(ctx) {
      ...
      const logs = await strapi
        ...
        .getLogsByModerator(parseInt(moderatorId), ctx.query);
    ...
    async findByContent(ctx) {
      ...
      const logs = await strapi
        .service('api::moderation-log.moderation-log')
        .getLogsByContent(contentType, parseInt(contentId));
     ...
     */
    async create(ctx) {
      const { action, contentType, contentId, reason, metadata } =
        ctx.request.body;
      ...
      if (
        ...
        return ctx.forbidden(
          'Você não tem permissão para criar logs de moderação',
        );
      }

      if (!action || !contentType || !contentId) {
        return ctx.badRequest(
          'action, contentType e contentId são obrigatórios',
        );
      }

      if (
        !['approved', 'rejected', 'user_promoted', 'user_demoted'].includes(
          action,
        )
      ) {
